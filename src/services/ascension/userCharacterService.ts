...
export const updateUserCharacter = async (
  ...
  if (dto.current_level !== undefined) {
    const stage = await levelToStageNumber("character", characterId, dto.current_level);
    if (stage !== null) updates.current_ascension_stage = stage;
  }
  if (dto.target_level !== undefined) {
    const stage = await levelToStageNumber("character", characterId, dto.target_level);
