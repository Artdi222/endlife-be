  ...
  try {
    const result = await pool.query<Character>(
      ...
      INSERT INTO characters
        (name, rarity, element, weapon_type, race, faction, description, order_index)
...
export const uploadCharacterMedia = async (
  ...
  try {
    // Determine bucket — images go to "characters", videos go to "videos"
    const isVideo = field === "video_enter" || field === "video_idle";
    const bucket = isVideo ? "videos" : "characters";

    // Map field to its subfolder
...
export const deleteCharacter = async (id: number): Promise<boolean> => {
  try {
    // Clean up Supabase storage — check each subfolder
